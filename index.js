...
async function startQasimBot() {
	...
	const getMessage = async (key) => {
		...
		return {
			...
			conversation: 'Hello From QUEEN SIA V1'
	...
	if (pairingCode && !phoneNumber && !qasim.authState.creds.registered) {
		...
		async function getPhoneNumber() {
			...
			if (!parsePhoneNumber('+' + phoneNumber).valid && phoneNumber.length < 6) {
				...
				console.log(chalk.bgBlack(chalk.redBright('Start with your Country WhatsApp code') + chalk.whiteBright(',') + chalk.greenBright(' Example : 254xxx')));
	...
	qasim.ev.on('connection.update', async (update) => {
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				console.log('Scan again and Run QUEEN...');
				...
				console.log('Connection Failure, Scan again and Run QUEEN...');
